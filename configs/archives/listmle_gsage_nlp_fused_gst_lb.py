...
class Configs(Base):
    OUTPUTDIR="../workdir/listmle_graphsage_nlp_fused_pairnorm_gst_b32"

    TRAIN_DATA_PATH="/app/dataset/various_splits/nlp_fused/train/*/"
    VALID_DATA_PATH="/app/dataset/various_splits/nlp_fused/valid"
    TEST_DATA_PATH="/app/dataset/various_splits/nlp_fused/test"
    ...
    AUTOCAST=False
    GRADIENT_STEPS=1
    VALIDATION_FREQUENCY=5   # Number of epochs
    ...
    def __init__(self,inference_files=None,inference_text=None,use_numpy=False):
        ...
        self.train_dataset = RandomDefaultFusedNpzDataset(self.TRAIN_DATA_PATH,min_configs=self.MIN_CONFIGS, max_configs=self.SAMPLE_CONFIGS,normalizers=self.NORMALIZER_PATH,sample_num=self.USE_DATASET_LEN,transforms=self.transforms,histogram_sampling=True,gst_training=0)
