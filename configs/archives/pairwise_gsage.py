...
class Configs(Base):
    OUTPUTDIR="../workdir/pairwise_graphsage"
    ...
    def __init__(self,inference_files=None,inference_text=None,use_numpy=False):
        ...
        self.criterion = CustomMSELoss(padding=self.TARGET_PADDING)
        print("Using MSE loss.")
        # self.criterion = listMLE


        self.dataloder_collate = GraphCollator(max_configs=self.SAMPLE_CONFIGS,configs_padding=self.CONFIG_PADDING,runtime_padding=self.RUNTIME_PADDING,provide_pair_matrix=self.IS_PAIR_TRAINING,mask_invalid_value=self.TARGET_PADDING)