...
def dense_to_sparse(dense_array):
    # Find the non-zero indices and values in the dense array
    nonzero_indices = np.transpose(np.nonzero(dense_array))
    nonzero_values = dense_array[nonzero_indices[:, 0], nonzero_indices[:, 1]]

    # Create a SparseTensor from the non-zero values and indices
    sparse_tensor = tf.sparse.SparseTensor(
        indices=nonzero_indices,
        values=nonzero_values,
        dense_shape=dense_array.shape
    )

    return sparse_tensor

def stack_sparse_tensors(sparse_tensors, axis=0):
    """
    Stack a list of sparse TensorFlow tensors along the specified axis.

    Args:
        sparse_tensors: A list of sparse TensorFlow tensors.
        axis: The axis along which to stack the tensors (0 for vertical stacking, 1 for horizontal stacking).

    Returns:
        A single sparse TensorFlow tensor containing the stacked tensors.
    """
    if axis not in [0, 1]:
        raise ValueError("Axis must be 0 (vertical stacking) or 1 (horizontal stacking).")

    # Use tf.sparse.concat to stack the sparse tensors
    stacked_sparse_tensor = tf.sparse.concat(axis=axis, sp_inputs=sparse_tensors)

    return stacked_sparse_tensor
...
class NpzDatasetPartition:
  ...
  def add_npz_file(
    ...
    for key, ndarray in npz_data.items():
      if key in {"node_feat","node_config_feat"}:
        ndarray = dense_to_sparse(ndarray)
  ...
  def finalize(self):
    ...
    gc.collect()
    self.node_feat = stack_sparse_tensors(self._data_dict.pop('node_feat'), axis=0)
    gc.collect()
    ...
    self.node_config_feat = stack_sparse_tensors(
  ...
  def get_item(self, index: int) -> LayoutExample:
    ...
    flat_config = tf.sparse.to_dense(
              tf.sparse.slice(
                  self.node_config_feat,
                  start=[flat_config_start, 0],  # Start index (row, column)
                  size=[flat_config_end-flat_config_start, self.node_config_feat.shape[-1]]  # Size (number of rows, number of columns)
              )
          )
    node_f = tf.sparse.to_dense(
          tf.sparse.slice(
              self.node_feat,
              start=[node_start, 0],  # Start index (row, column)
              size=[node_end-node_start, self.node_feat.shape[-1]]  # Size (number of rows, number of columns)
          )
      )
